...
class Options : OptionGroup("Main Options:") {
    ...
    val waitFor: Int? by option(
        "--wait-for",
        help = "Wait for the specified call before starting."
    ).int()
}
...
class Entrypoint : CliktCommand() {
    ...
    override fun run() {
        ...
        val wait = options.waitFor != null

        Server().start(
            port = options.port ?: config[EnvSpec.port],
            wait = wait
        )
